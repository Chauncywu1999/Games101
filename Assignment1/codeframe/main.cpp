...
Eigen::Matrix4f get_model_matrix(float rotation_angle)
{
    ...
    // Then return it.
	float cosa = cos(rotation_angle * MY_PI / 180);
    float sina = sin(rotation_angle * MY_PI / 180);
...
Eigen::Matrix4f get_rotation(Eigen::Vector3f axis, float angle)
{
	...
	Eigen::Vector3f ut = axis.normalized();
	//std::cout << ut << std::endl;
	Eigen::Matrix3f rk;
	Eigen::Matrix3f M;
	Eigen::Matrix4f model;
	rk << 0, -ut[2], ut[1], ut[2], 0, -ut[0], -ut[1], ut[0], 0;
	M = Eigen::Matrix3f::Identity() + (1 - cos(a)) * rk * rk + rk * sin(a);
